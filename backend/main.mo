...
import AccessControl "authorization/access-control";
 ...
 actor {
  ...
  type UserProfile = {
    name : Text;
    isAuthor : Bool; // Track author status within user role
  };
  ...
  func isAuthor(user : Principal) : Bool {
    switch (userProfiles.get(user)) {
      case (null) { false };
      case (?profile) { profile.isAuthor };
    };
  };

  // Helper function to check if user owns a book
  func ownsBook(user : Principal, book : Book) : Bool {
    book.uploaderId == user;
  };
  ...
  public query ({ caller }) func getUserProfile(user : Principal) : async ?UserProfile {
    // Anyone can view public author profiles, but only self or admin can view full profile
    switch (userProfiles.get(user)) {
      case (null) { null };
      case (?profile) {
        if (profile.isAuthor) {
          // Public author profile - anyone can view
          ?profile;
        } else {
          // Private profile - only self or admin
          if (caller != user and not AccessControl.isAdmin(accessControlState, caller)) {
            Runtime.trap("Unauthorized: Can only view your own profile or public author profiles");
          };
          ?profile;
        };
      };
  ...
  public shared ({ caller }) func saveCallerUserProfile(profile : UserProfile) : async () {
    ...
    // Prevent users from self-promoting to author
    let existingProfile = userProfiles.get(caller);
    let finalProfile = switch (existingProfile) {
      case (null) {
        // New profile - cannot set isAuthor to true without admin approval
        { profile with isAuthor = false };
      };
      case (?existing) {
        // Existing profile - preserve isAuthor status unless admin
        if (AccessControl.isAdmin(accessControlState, caller)) {
          profile;
        } else {
          { profile with isAuthor = existing.isAuthor };
        };
      };
    };
  ...
  public shared ({ caller }) func promoteToAuthor(user : Principal) : async () {
    if (not (AccessControl.isAdmin(accessControlState, caller))) {
      Runtime.trap("Unauthorized: Only admins can promote users to author status");
    };
    ...
    switch (userProfiles.get(user)) {
      ...
      case (?profile) {
        userProfiles.add(user, { profile with isAuthor = true });
      };
    };
  };

  public query ({ caller }) func getUserRole(user : Principal) : async { systemRole : UserRole; isAuthor : Bool } {
    if (caller != user and not AccessControl.isAdmin(accessControlState, caller)) {
      Runtime.trap("Unauthorized: Can only view your own role");
    };

    let systemRole = AccessControl.getUserRole(accessControlState, user);
    let authorStatus = isAuthor(user);

    { systemRole; isAuthor = authorStatus };
  };
  ...
  public shared ({ caller }) func submitBookForApproval(book : Book) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("Unauthorized: Only authenticated users can submit books");
    };

    // Only authors and admins can submit books
    if (not (isAuthor(caller) or AccessControl.isAdmin(accessControlState, caller))) {
      Runtime.trap("Unauthorized: Only authors and admins can submit books");
    };
  ...
  public shared ({ caller }) func updateBook(isbn : Text, updatedBook : Book) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("Unauthorized: Only authenticated users can update books");
    };

    switch (books.get(isbn)) {
      case (null) { Runtime.trap("Book not found") };
      case (?existingBook) {
        // Admins can edit any book without restrictions
        if (AccessControl.isAdmin(accessControlState, caller)) {
          books.add(
            isbn,
            { updatedBook with uploaderId = existingBook.uploaderId; createdAt = existingBook.createdAt; editCount = existingBook.editCount },
          );
        } else {
          // Authors can only edit their own books
          if (not isAuthor(caller)) {
            Runtime.trap("Unauthorized: Only authors can edit books");
          };

          if (not ownsBook(caller, existingBook)) {
          ...
          let newEditCount = existingBook.editCount + 1;
          books.add(
            isbn,
  ...
  public shared ({ caller }) func deleteBook(isbn : Text) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("Unauthorized: Only authenticated users can delete books");
    };
    ...
    switch (books.get(isbn)) {
      ...
      case (?book) {
        ...
        if (AccessControl.isAdmin(accessControlState, caller)) {
          ...
          if (not isAuthor(caller)) {
            Runtime.trap("Unauthorized: Only authors can delete books");
          };

          if (not ownsBook(caller, book)) {
  ...
  public query ({ caller }) func getBook(isbn : Text) : async ?Book {
    // All users including guests can view approved books
    switch (books.get(isbn)) {
      case (null) { null };
      case (?book) {
        switch (book.approvalStatus) {
          case (#approved) { ?book };
          case (_) {
            // Only admins and book owner can see non-approved books
            if (AccessControl.isAdmin(accessControlState, caller) or ownsBook(caller, book)) {
              ?book;
            } else {
              null;
            };
  ...
  public query ({ caller }) func getAllBooks() : async [Book] {
    // Guests and regular users see only approved books
    // Authors see their own books plus approved books
    // Admins see all books
    if (AccessControl.isAdmin(accessControlState, caller)) {
      books.values().toArray();
    } else if (isAuthor(caller)) {
      books.values().filter(
        func(b) {
          b.approvalStatus == #approved or ownsBook(caller, b);
        }
      ).toArray();
    } else {
      books.values().filter(
        func(b) { b.approvalStatus == #approved }
      ).toArray();
    };
  };
  ...
  public query ({ caller }) func getBooksByGenre(genre : Text) : async [Book] {
    if (AccessControl.isAdmin(accessControlState, caller)) {
      ...
      books.values().filter(
        func(b) {
          b.genre == genre and (b.approvalStatus == #approved or ownsBook(caller, b));
        }
      ).toArray();
    } else {
      books.values().filter(
        func(b) {
          b.genre == genre and b.approvalStatus == #approved;
        }
      ).toArray();
    };
  };
  ...
  public query ({ caller }) func getBooksByAuthor(author : Text) : async [Book] {
    if (AccessControl.isAdmin(accessControlState, caller)) {
      ...
      books.values().filter(
        func(b) {
          b.author == author and (b.approvalStatus == #approved or ownsBook(caller, b));
        }
      ).toArray();
    } else {
      books.values().filter(
        func(b) {
          b.author == author and b.approvalStatus == #approved;
        }
      ).toArray();
    };
  };
  ...
  public query ({ caller }) func getBooksByGenreAndAuthor(genre : Text, author : Text) : async [Book] {
    if (AccessControl.isAdmin(accessControlState, caller)) {
      ...
      books.values().filter(
        func(b) {
          b.genre == genre and b.author == author and (b.approvalStatus == #approved or ownsBook(caller, b));
        }
      ).toArray();
    } else {
      books.values().filter(
  ...
  public shared ({ caller }) func updateReadingProgress(bookIsbn : Text, pagesRead : Nat) : async () {
    ...
    switch (books.get(bookIsbn)) {
      case (null) { Runtime.trap("Book not found") };
      case (?book) {
        // Only allow progress updates for approved books (unless admin/owner)
        if (not (book.approvalStatus == #approved or AccessControl.isAdmin(accessControlState, caller) or ownsBook(caller, book))) {
          Runtime.trap("Cannot track progress for non-approved books");
        };
  ...
  public query ({ caller }) func getReadingProgress(user : Principal, bookIsbn : Text) : async ?ReadingProgress {
    // Users can view their own progress, admins can view any progress
    if (caller != user and not AccessControl.isAdmin(accessControlState, caller)) {
  ...
  public query ({ caller }) func getUserBookProgress(bookIsbn : Text) : async ?ReadingProgress {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("Unauthorized: Only authenticated users can view their progress");
    };
    let progressKey = buildReadingProgressKey(caller, bookIsbn);
    readingProgressMap.get(progressKey);
  };
  ...
  public query ({ caller }) func getBookCompletedUsers(bookIsbn : Text) : async [Principal] {
    // Only admins and book owners can see completion analytics
    switch (books.get(bookIsbn)) {
      case (null) { Runtime.trap("Book not found") };
      case (?book) {
        if (not (AccessControl.isAdmin(accessControlState, caller) or ownsBook(caller, book))) {
          Runtime.trap("Unauthorized: Only admins and book owners can view analytics");
        };
  ...
  public shared ({ caller }) func requestMoreEdits(isbn : Text, message : ?Text) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("Unauthorized: Only authenticated users can request edits");
    };

    switch (books.get(isbn)) {
      case (null) { Runtime.trap("Book not found") };
      case (?book) {
        // Only authors can request more edits for their own books
        if (not isAuthor(caller)) {
          Runtime.trap("Unauthorized: Only authors can request more edits");
        };

        if (not ownsBook(caller, book)) {
          Runtime.trap("Unauthorized: You can only request edits for your own books");
        };
  ...
  public shared ({ caller }) func toggleBookmark(bookIsbn : Text) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("Unauthorized: Only authenticated users can bookmark books");
    };

    // Verify book exists and is approved (or user is admin/owner)
    switch (books.get(bookIsbn)) {
      case (null) { Runtime.trap("Book not found") };
      case (?book) {
        if (not (book.approvalStatus == #approved or AccessControl.isAdmin(accessControlState, caller) or ownsBook(caller, book))) {
          Runtime.trap("Cannot bookmark non-approved books");
        };
      };
    };
  ...
  public query ({ caller }) func getBookmarkedBooks(user : Principal) : async [Book] {
    // Users can view their own bookmarks, admins can view any user's bookmarks
    if (caller != user and not AccessControl.isAdmin(accessControlState, caller)) {
  ...
  public query ({ caller }) func isBookBookmarked(user : Principal, bookIsbn : Text) : async Bool {
    // Users can check their own bookmarks, admins can check any user's bookmarks
    if (caller != user and not AccessControl.isAdmin(accessControlState, caller)) {
  ...
  public shared ({ caller }) func addRating(bookIsbn : Text, stars : Nat) : async () {
    ...
    switch (books.get(bookIsbn)) {
      case (null) { Runtime.trap("Book not found") };
      case (?book) {
        // Only allow ratings for approved books (unless admin/owner)
        if (not (book.approvalStatus == #approved or AccessControl.isAdmin(accessControlState, caller) or ownsBook(caller, book))) {
          Runtime.trap("Cannot rate non-approved books");
        };
  ...
  public query ({ caller }) func getBookRatings(bookIsbn : Text) : async [Rating] {
    // Public information for approved books
    switch (books.get(bookIsbn)) {
      case (null) { Runtime.trap("Book not found") };
      case (?book) {
        if (book.approvalStatus == #approved or AccessControl.isAdmin(accessControlState, caller) or ownsBook(caller, book)) {
  ...
  public query ({ caller }) func getTrendingBooks() : async [Book] {
    // Public information - show approved books to all users
    let currentTime = Time.now();
  ...
  public query ({ caller }) func getPersonalizedRecommendations(user : Principal) : async [Recommendation] {
    // Users can view their own recommendations, admins can view any user's recommendations
    if (caller != user and not AccessControl.isAdmin(accessControlState, caller)) {
    ...
    let availableBooks = if (AccessControl.isAdmin(accessControlState, caller)) {
      ...
      books.toArray().filter(
        func((_, book)) { book.approvalStatus == #approved or ownsBook(caller, book) }
      );
    } else {
      books.toArray().filter(
